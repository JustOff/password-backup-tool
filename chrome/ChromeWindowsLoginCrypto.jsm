...
Cu.import("chrome://pwdbackuptool/content/OSCrypto.jsm");
Cu.import("resource://gre/modules/XPCOMUtils.jsm");

var ChromeMigrationUtils = {
  /**
   * Get the local state file content.
   * @param {String} dataPath the path to the 'Login Data' file
   * @returns {Object} The JSON-based content.
   */
  async getLocalState(dataPath) {
    let localState = null;
    try {
      let localStatePath = OS.Path.join(
        OS.Path.dirname(OS.Path.dirname(dataPath)),
        "Local State"
      );
